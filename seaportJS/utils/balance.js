...
var balanceOf = function (owner, item, multicallProvider, criteria) {
      ...
      if ((0, item_1.isErc721Item)(item.itemType)) {
        ...
        if (item.itemType === constants_1.ItemType.ERC721_WITH_CRITERIA) {
          return [
            2 /*return*/,
            criteria
              ? contract.ownerOf(criteria.identifier).then(function (ownerOf) {
                  return ethers_1.BigNumber.from(
        ...
        return [
          2 /*return*/,
          contract.ownerOf(item.identifierOrCriteria).then(function (ownerOf) {
            return ethers_1.BigNumber.from(
              Number(ownerOf.toLowerCase() === owner.toLowerCase())
            );
          }),
        ...
        if (item.itemType === constants_1.ItemType.ERC1155_WITH_CRITERIA) {
          if (!criteria) {
            ...
            return [
              2 /*return*/,
              startAmount.gt(endAmount) ? startAmount : endAmount,
            ];
          }
        ...
        return [
          2 /*return*/,
          contract.balanceOf(owner, item.identifierOrCriteria),
        ];
      }
